...
;   case 4:  zero cap_a, expect -1
;   case 5:  zero cap_b, expect -1
;   case 6:  len > cap_a, expect -1
;   case 7:  len > cap_b, expect -1
;   case 8:  len == cap_a == cap_b (exact fit), expect 0 and correct result
;   case 9: len < cap_a and len < cap_b, expect 0 and correct result
;   case 10: valid large buffer, expect 0 and correct result
;   case 11: lhs == rhs (same pointer), expect 0 and buffer unchanged
;   case 12: unaligned lhs, expect 0 and correct result
;   case 13: tail test (7 bytes), expect 0 and correct result
...
SYM(main):
    ...
    ; ---- case 8: len == cap_a == cap_b (exact fit), expect 0 and swapped ----
    push    small_size                      ; len (16)
...
.verify_case8_rhs:
    ...
    inc     esi
    dec     ecx
    jnz     .verify_case8_rhs

    ; ---- case 9: len < cap_a and len < cap_b, expect 0 and swapped ----
...
.verify_case9_rhs:
    ...
    ; ---- case 10: valid large buffer, expect 0 and swapped ----
...
.verify_case11:
    ...
    ; ---- case 12: unaligned lhs (lhs+1), expect 0 and swapped ----
    push    align_size                      ; len
...
.verify_case12_rhs:
    ...
    inc     esi
    dec     ecx
    jnz     .verify_case12_rhs

    ; ---- case 13: tail test (7 bytes), expect 0 and swapped ----
